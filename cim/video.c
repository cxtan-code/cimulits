...
void init_device(struct display_info *disp)
{
	...
	if (0 == xioctl(fd, VIDIOC_CROPCAP, &cropcap)) {
		if (1 == disp->do_crop){
			...
			crop.c.left = disp->crop_x;
			crop.c.top = disp->crop_y;
			crop.c.width = disp->crop_w;
			crop.c.height = disp->crop_h;
			printf("use owen crop set\n");
		}else{
			crop.c = cropcap.defrect;	/* reset to default */
			disp->crop_x = crop.c.left;
			disp->crop_y = crop.c.top;
			disp->crop_w = crop.c.width;
			disp->crop_h = crop.c.height;
		}
		if (1 == disp->do_crop){
			if (-1 == xioctl(fd, VIDIOC_S_CROP, &crop)) {
				fprintf(stderr, "%s is crop faild\n", dev_name);
				exit(EXIT_FAILURE);
			}
		}
	} else {
		fprintf(stderr, "%s is not support crop\n", dev_name);
		exit(EXIT_FAILURE);
	}
	switch (io) {
	case IO_METHOD_READ:
